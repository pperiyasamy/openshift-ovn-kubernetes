...
func (c *Controller) Start() error {
	...
	if err != nil {
		return fmt.Errorf("failed to register a NAD reconciler: %v", err)
	}
...
func (c *Controller) discoverUnmanagedVTEPIPs(vtep *vtepv1.VTEP, node *corev1.Node) (net.IP, net.IP, error) {
	...
	ipv4, err := c.pickVTEPIP(v4Matches, cidrs, netlink.FAMILY_V4)
	if err != nil {
		return nil, nil, err
	}
	ipv6, err := c.pickVTEPIP(v6Matches, cidrs, netlink.FAMILY_V6)
	if err != nil {
		return nil, nil, err
	}
...
func (c *Controller) pickVTEPIP(matches []net.IP, vtepCIDRs []*net.IPNet, family int) (net.IP, error) {
	if len(matches) <= 1 {
		if len(matches) == 1 {
	...
	for _, addr := range addrs {
		...
		if util.IsIPContainedInAnyCIDR(addr.IP, vtepCIDRs...) {
