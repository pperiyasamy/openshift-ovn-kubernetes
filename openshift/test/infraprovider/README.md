...
| - EgressIP targets         |  2001:db8:abcd::/64  |                        |
|   (podman)                 |                      |                        |
|                            |                      |                        |
...
- **[1]** Skipped for UDN network types only (has `Feature:NetworkSegmentation`
  label). Cluster Default variants run on GCP.
...
- **[3]** Skipped on GCP for ALL network types. On GCP, nodes have `/32` primary
  ...
  verification. Tracked via
  [OCPBUGS-122016](https://redhat.atlassian.net/browse/OCPBUGS-122016).
...
| EgressIP failover    | EgressIPs cannot float between nodes in different subnets      |
+----------------------+----------------------------------------------------------------+
```
